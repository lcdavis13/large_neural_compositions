...
def load_and_align_data(file_path):
    ...
    Retain the lowest test error value for all subsequent epochs after reaching it.
    ...
    for fold in folds:
        ...
        # Find the best (minimum) loss values and propagate them forward
        train_loss = fold_data['Avg Training Loss'].cummin().reset_index(drop=True)
        val_loss = fold_data['Avg Validation Loss'].cummin().reset_index(drop=True)

        # Update max_length based on the length of each fold's data
        max_length = max(max_length, len(train_loss), len(val_loss))

        # Append the processed loss data
